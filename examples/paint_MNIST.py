...
def main(args): 
    ...
    unlearned_model = torch.load('/home/cwh/Workspace/TorchLRP-master/examples/models/mnist_unlearn_class_epsilon_1.pkl').to(args.device)
    
    train_loader, test_loader = get_mnist_data(transform=torchvision.transforms.ToTensor(), batch_size=1)
    ...
    train_loader = DataLoader(filtered_data, batch_size=args.batch_size, shuffle=True)
    ...
    def compute_and_plot_explanation(test_model, rule, ax_, title=None, postprocess=None, pattern=None, cmap='seismic'): 
        ...
        y_hat = test_model.forward(x, explain=True, rule=rule, pattern=pattern)
        # Choose argmax
    ...
    compute_and_plot_explanation(model, "gradient", ax[1, 0], title="input$\\times$gradient", postprocess = lambda attribution: attribution * x)

    compute_and_plot_explanation(model, "epsilon", ax[0, 1])
    compute_and_plot_explanation(model, "gamma+epsilon", ax[1, 1], title="  gamma+epsilon")
# 
    compute_and_plot_explanation(model, "alpha1beta0", ax[0, 2])
    compute_and_plot_explanation(model, "alpha2beta1", ax[1, 2])
# 
    compute_and_plot_explanation(model, "patternnet", ax[0, 3], pattern=patterns_all, title="PatternNet \n$S(x)$", cmap='gray')
    compute_and_plot_explanation(model, "patternnet", ax[1, 3], pattern=patterns_pos, title="PatternNet \n$S(x)_{+-}$", cmap='gray')

    compute_and_plot_explanation(model, "patternattribution", ax[0, 4], pattern=patterns_all, title="PatternAttribution \n$S(x)$")
    compute_and_plot_explanation(model, "patternattribution", ax[1, 4], pattern=patterns_pos, title="PatternAttribution \n$S(x)_{+-}$")

    fig.tight_layout()

    fig.savefig((base_path / 'examples' / 'plots' / "mnist_explanations_origin_else.png").as_posix(), dpi=800)
    # plt.show()
    ...
    with torch.no_grad(): 
        x_plot = heatmap_grid(x*2-1, cmap_name="gray")
        plot_attribution(x_plot, ax1[0, 0], pred, "Input")

    # compute_and_plot_explanation("gradient", ax[1, 0], title="gradient")
    compute_and_plot_explanation(unlearned_model, "gradient", ax1[1, 0], title="input$\\times$gradient", postprocess = lambda attribution: attribution * x)

    compute_and_plot_explanation(unlearned_model, "epsilon", ax1[0, 1])
    compute_and_plot_explanation(unlearned_model, "gamma+epsilon", ax1[1, 1], title="  gamma+epsilon")
# 
    compute_and_plot_explanation(unlearned_model, "alpha1beta0", ax1[0, 2])
    compute_and_plot_explanation(unlearned_model, "alpha2beta1", ax1[1, 2])
# 
    compute_and_plot_explanation(unlearned_model, "patternnet", ax1[0, 3], pattern=patterns_all, title="PatternNet \n$S(x)$", cmap='gray')
    compute_and_plot_explanation(unlearned_model, "patternnet", ax1[1, 3], pattern=patterns_pos, title="PatternNet \n$S(x)_{+-}$", cmap='gray')

    compute_and_plot_explanation(unlearned_model, "patternattribution", ax1[0, 4], pattern=patterns_all, title="PatternAttribution \n$S(x)$")
    compute_and_plot_explanation(unlearned_model, "patternattribution", ax1[1, 4], pattern=patterns_pos, title="PatternAttribution \n$S(x)_{+-}$")

    fig1.tight_layout()

    fig1.savefig((base_path / 'examples' / 'plots' / "mnist_explanations_unlearn_else.png").as_posix(), dpi=280)
