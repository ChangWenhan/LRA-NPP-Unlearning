...
dataset = ImageNetDataset(root_dir='/home/cwh/Workspace/TorchLRP-master/torch_imagenet/imagenet-mini/train/n01608432', transform=transform)
train_loader = torch.utils.data.DataLoader(dataset, batch_size=9, shuffle=False)
...
vgg = torch.load("examples/models/imagenet_unlearned_class.pkl").to(device)
vgg.eval()
...
fig.tight_layout()
fig.savefig((base_path / 'examples' / 'plots' / ("vgg%i_explanations_unlearn_else.png" % vgg_num)).as_posix(), dpi=280)
# plt.show()