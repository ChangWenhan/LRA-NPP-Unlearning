import os
from copy import deepcopy


BASE_DIR = "/home/cwh/Workspace/TorchLRP-master"


def _p(*parts):
    return os.path.join(BASE_DIR, *parts)


DATASET_CONFIGS = {
    "mnist": {
        "dataset": "mnist",
        "unlearn_class": 1,
        "model_path": _p("examples", "models", "mnist_model.pth"),
        "output_root": _p("examples", "batch_experiments", "outputs"),
        "batch_size": 64,
        "analysis_sample_size": 36,
        "analyze_top_n": 50,
        "perturb_top_n": 80,
        "rules": ["epsilon"],
        "input_noise": 0.0,
        "n_runs": 5,
        "seed_base": 42,
        "seed_stride": 100,
        "methods": ["retrain", "lra_npp", "noise_gn", "noise_ln"],
        "noise_std": 1.0,
        "noise_laplace_scale": 1.0,
        "retrain": {
            "epochs": 5,
            "lr": 1e-2,
            "optimizer": "sgd",
            "momentum": 0.9,
            "from_scratch": True,
        },
        "layer_map": {
            "analysis_layer": 0,
            "output_layer_idx": 8,
            "secondary_layer_idx": 6,
            "target_class_only": True,
        },
        "save_models": False,
    },
    "cifar10": {
        "dataset": "cifar10",
        "unlearn_class": 9,
        "model_path": _p("examples", "models", "resnet50_cifar10_epoch_10.pth"),
        "output_root": _p("examples", "batch_experiments", "outputs"),
        "batch_size": 32,
        "analysis_sample_size": 36,
        "analyze_top_n": 200,
        "perturb_top_n": 400,
        "rules": ["epsilon"],
        "input_noise": 0.0,
        "n_runs": 5,
        "seed_base": 42,
        "seed_stride": 100,
        "methods": ["retrain", "lra_npp", "noise_gn", "noise_ln"],
        "noise_std": 1.0,
        "noise_laplace_scale": 1.0,
        "retrain": {
            "epochs": 10,
            "lr": 1e-3,
            "optimizer": "sgd",
            "momentum": 0.9,
            "from_scratch": True,
            "ag_stop_target": 0.955,
        },
        "layer_map": {
            "analysis_layer": 0,
            "output_layer_idx": 22,
            "secondary_layer_idx": None,
            "target_class_only": False,
        },
        "save_models": False,
    },
    "cifar100": {
        "dataset": "cifar100",
        "unlearn_class": 0,
        "model_path": _p("examples", "models", "resnet50_cifar100_5.pth"),
        "output_root": _p("examples", "batch_experiments", "outputs"),
        "batch_size": 64,
        "analysis_sample_size": 36,
        "analyze_top_n": 150,
        "perturb_top_n": 250,
        "rules": ["epsilon"],
        "input_noise": 0.0,
        "n_runs": 5,
        "seed_base": 42,
        "seed_stride": 100,
        "methods": ["retrain", "lra_npp", "noise_gn", "noise_ln"],
        "noise_std": 1.0,
        "noise_laplace_scale": 1.0,
        "retrain": {
            "epochs": 10,
            "lr": 1e-3,
            "optimizer": "sgd",
            "momentum": 0.9,
            "from_scratch": True,
            "scheduler": {"type": "step", "step_size": 30, "gamma": 0.1},
            "ag_stop_target": 0.80,
        },
        "layer_map": {
            "analysis_layer": 0,
            "output_layer_idx": 22,
            "secondary_layer_idx": None,
            "target_class_only": False,
        },
        "save_models": False,
    },
    "imagenet": {
        "dataset": "imagenet",
        "unlearn_class": 0,
        "batch_size": 32,
        "analysis_sample_size": 36,
        "analyze_top_n": 150,
        "perturb_top_n": 400,
        "rules": ["epsilon"],
        "input_noise": 0.0,
        "n_runs": 5,
        "seed_base": 42,
        "seed_stride": 100,
        "noise_std": 1.0,
        "noise_laplace_scale": 1.0,
        "retrain": {
            "epochs": 50,
            "lr": 1e-3,
            "optimizer": "sgd",
            "momentum": 0.9,
            "from_scratch": True,
            "ag_stop_target": 0.80,
        },
        "imagenet_unlearn_data_dir": _p("torch_imagenet", "imagenet-mini", "train", "n01440764"),
        "imagenet_full_data_dir": _p("torch_imagenet", "imagenet-mini", "train"),
        "imagenet_test_data_dir": _p("torch_imagenet", "imagenet-mini", "val"),
        "vgg_version": 16,
        "use_pretrained": True,
        "mia_supplement_nonmember_with_other_test_classes": True,
        "layer_map": {
            "analysis_layer": 0,
            "fc1_layer_idx": 36,
            "fc2_layer_idx": 39,
            "output_layer_idx": 42,
            "perturb_fc1": True,
            "perturb_fc2": True,
            "perturb_output": False,
        },
        "output_root": _p("examples", "batch_experiments", "outputs"),
        "methods": ["retrain", "lra_npp", "noise_gn", "noise_ln"],
        "save_models": False,
    },
    "mufac": {
        "dataset": "mufac",
        "unlearn_class": 0,
        "batch_size": 64,
        "analysis_sample_size": 36,
        "analyze_top_n": 400,
        "perturb_top_n": 500,
        "rules": ["epsilon"],
        "lrp_rule_type": "epsilon",
        "lrp_params": {
            "gamma": 0.25,
            "epsilon": 1e-6,
            "alpha": 2.0,
            "beta": 1.0,
        },
        "input_noise": 0.0,
        "n_runs": 5,
        "seed_base": 42,
        "seed_stride": 100,
        "noise_std": 1.0,
        "noise_laplace_scale": 1.0,
        "legacy_lra_npp_alignment": True,
        "retrain": {
            "epochs": 10,
            "lr": 1e-4,
            "optimizer": "adam",
            "from_scratch": True,
            "ag_stop_target": 0.85,
        },
        "mufac_root": _p("data", "custom_korean_family_dataset_resolution_128"),
        "mufac_model_path": _p("examples", "models", "vit_best_on_test.pth"),
        "mufac_num_classes": 8,
        "output_root": _p("examples", "batch_experiments", "outputs"),
        "methods": ["retrain", "lra_npp", "noise_gn", "noise_ln"],
        "save_models": False,
    },
}


def get_config(dataset_name: str):
    key = dataset_name.lower()
    if key not in DATASET_CONFIGS:
        raise ValueError(f"Unknown dataset: {dataset_name}")
    return deepcopy(DATASET_CONFIGS[key])
