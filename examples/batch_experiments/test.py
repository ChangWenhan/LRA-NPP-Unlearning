...
EXPERIMENT_CONFIG = {
    ...
    # [新增] 神经元保存功能
    'save_neurons': True,    # <--- 修改为 True 以开启保存
    'neuron_save_dir': '/home/cwh/Workspace/TorchLRP-master/neuron/mufac', # <--- 保存路径
...
def calculate_statistics(current_values, baseline_value):
    """
    计算一组实验结果的统计指标
    Returns: dict 包含 mean, std, p_value, cohens_d, cliffs_delta
    """
...
def calculate_mia_score(model, train_loader, test_loader, target_class, device):
    ...
    Member (1): 训练集中的 target_class
    Non-Member (0): 测试集中的 target_class
    """
...
def run_full_experiment_suite(config):
    ...
    for (n_samples, noise, k, h) in combinations:
        ...
        for run_id in range(1, config['n_repeats'] + 1):
            ...
            # ================= [修改] 保存神经元逻辑 =================
            if config.get('save_neurons', False) and run_id == 1:
                neuron_save_path = Path(config['neuron_save_dir'])
                neuron_save_path.mkdir(parents=True, exist_ok=True)
                
                # 文件名包含所有参数，特别是 H 和 run_id
                fname = f"class{config['unlearn_class']}_S{n_samples}_N{noise}_K{k}_H{h}_run{run_id}.pkl"
                
                with open(neuron_save_path / fname, 'wb') as f:
                    pickle.dump(high_freq_neurons, f)
                    sys.exit()
            # =======================================================
