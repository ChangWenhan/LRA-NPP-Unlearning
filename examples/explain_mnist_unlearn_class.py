...
def compute_and_plot_explanation(rule, test_loader, dataloader, prepared_model, ax_=None, title=None, postprocess=None, pattern=None, cmap='seismic'): 
    ...
    for x, y in dataloader:
        ...
        for i,t in enumerate(all_relevances):
            t = t[0].tolist()
            print(len(t))
            index = sorted(range(len(t)), key=lambda i: t[i], reverse=True)[:50]
    ...
    # 按照出现次数降序排列数字
    sorted_numbers = sorted(number_counts.items(), key=lambda x: x[1], reverse=True)[:80]
    ...
    # with open('neuron/mnist/{}.pkl'.format(unlearn_class), 'wb') as f:
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            correct += (predicted == labels).sum().item()
            # 统计类别1的样本精度
            class1_mask = (labels == unlearn_class)  # 类别1的样本掩码
            class1_total += class1_mask.sum().item()
            class1_correct += ((predicted == labels) & class1_mask).sum().item()
            
            classElse_mask = (labels != unlearn_class)  # 类别1的样本掩码
    ...
    print('Accuracy on test images of unlearned class: %.3f %%' % (100 * class1_correct / class1_total))
...
def main(args): 
    ...
    for data, labels in test_loader:
        ...
        if (labels == unlearn_class).any():  # 如果labels是tensor
