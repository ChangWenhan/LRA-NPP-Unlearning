...
# 找到类别 0 的索引
class_0_indices = [i for i, (_, label) in enumerate(full_testset) if label == unlearn_class]

# 创建类别 0 的子集
class_0_subset = Subset(full_testset, class_0_indices)

# 创建 train_loader 和 test_loader
train_loader = DataLoader(class_0_subset, batch_size=8, shuffle=True)
test_loader = DataLoader(full_testset, batch_size=64, shuffle=False)

vgg = torch.load('examples/models/resnet50_cifar100_5.pth').to(device)
vgg.eval()
...
def compute_and_plot_explanation_multiAnalysis(perturbed_num, rule, ax_, patterns=None, plt_fn=heatmap_grid): 
    ...
    # 保存到文件
    # with open('neuron/cifar-100/{}.pkl'.format(unlearn_class), 'wb') as f:
    ...
    # torch.save(model, "/home/cwh/Workspace/TorchLRP-master/examples/models/resnet50_cifar100_unlearned.pth")
...
for i, (rule, pattern, fn, (p, q) ) in enumerate(explanations): 
    ...
    for num in perturbed_num_list:
        ...
        lrp_vgg = lrp.convert_vgg(vgg).to(device)# fig.tight_layout()
# fig.savefig((base_path / 'examples' / 'plots' / ("CIFAR100_explanations1.png")).as_posix(), dpi=280)
# plt.show()