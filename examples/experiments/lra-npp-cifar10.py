...
EXPERIMENT_CONFIG = {
    ...
    # 每个样本分析的神经元数量列表
    'analyze_top_n_list': [200],
    
    # 最终扰动的神经元数量列表
    'perturb_top_n_list': [400, 500, 600],
    ...
    'save_model': False,
    'model_save_path': 'examples/models/resnet50_cifar10_unlearned.pth'
}
...
def add_image_noise(images, noise_level):
    ...
    # 可选：限制在合理范围内
    # noisy_images = torch.clamp(noisy_images, -2.5, 2.5)  # 基于归一化范围
    return noisy_images
...
def load_data(config):
    ...
    return target_subset, train_loader
...
def run_single_experiment(rule, analyze_top_n, perturb_top_n, perturbation, 
                         ...
                         target_dataset, train_loader, device):
    """运行单个实验"""
    print("\n" + "="*70)
    print(f"实验配置:")
    ...
    # 验证模型
    print("\n模型验证结果:")
    validate_model(lrp_model, train_loader, device, target_class=base_config['unlearn_class'])
...
def main():
    ...
    print("\n" + "="*70)
    print("加载数据集...")
    target_dataset, train_loader = load_data(config)
    ...
    # 验证原始模型
    print("\n原始模型性能:")
    validate_model(original_model, train_loader, device, target_class=config['unlearn_class'])
