...
# 找到类别 0 的索引
class_0_indices = [i for i, (_, label) in enumerate(full_dataset) if label == unlearn_class]

# 创建类别 0 的子集
class_0_subset = Subset(full_dataset, class_0_indices)

# 创建 train_loader 和 test_loader
test_loader = DataLoader(class_0_subset, batch_size=1, shuffle=True)
...
train_loader = DataLoader(full_dataset, batch_size=64, shuffle=False)

vgg = torch.load('examples/models/resnet50_cifar10_epoch_10.pth').to(device)
vgg.eval()
...
# # # # # Plotting 
def compute_and_plot_explanation_multiAnalysis(perturbed_num, rule, ax_, patterns=None, plt_fn=heatmap_grid): 
    ...
    # 按照出现次数降序排列数字
    sorted_numbers = sorted(number_counts.items(), key=lambda x: x[1], reverse=True)[:perturbed_num]
    ...
    # 保存到文件
    with open('neuron/cifar-10/{}.pkl'.format(unlearn_class), 'wb') as f:
        pickle.dump(sorted_numbers, f)
    sys.exit()
    ...
    # for class_num in range(10):
    #     for i in sorted_numbers:
    #         fc_weights[class_num][i] = add_gaussian_noise(fc_weights[class_num][i], mean=0.0, std=1.0)
    # lrp_vgg[22].weight.data = fc_weights

    end_time = time.time()
    print("Time for unlearning: ", end_time - start_time)

    validate_model(model=lrp_vgg, data_loader=train_loader, device=device)

    model = copy.deepcopy(lrp_vgg)
    # torch.save(model, "/home/cwh/Workspace/TorchLRP-master/examples/models/resnet50_cifar10_gn.pth")
...
# validate_model(model=lrp_vgg, data_loader=train_loader, device=device)
perturbed_num_list = [400, 500, 600]
# Plot explanations
for i, (rule, pattern, fn, (p, q) ) in enumerate(explanations): 
    for num in perturbed_num_list:
        compute_and_plot_explanation_multiAnalysis(num, rule, ax[p, q], patterns=pattern, plt_fn=fn)
        lrp_vgg = lrp.convert_vgg(vgg).to(device)
# fig.tight_layout()
